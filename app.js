...
var driver = neo4j.driver("bolt://localhost", neo4j.auth.basic("neo4j", "111111111"))
...
// Acquaintances Connect Route
app.post('/acquaintances/connect', function(req, res){
    var nameOne = req.body.nameOne
    var nameSecond = req.body.nameSecond
    var id = req.body.id
    
    session
        .run("MATCH(a:Person {name:{nameParam1}}),(b:Person {name:{nameParam2}}) MERGE(a)-[r:ACQUAINTANCES]->(b) RETURN a,b", {nameParam1: nameOne, nameParam2:nameSecond})
        .then(function(result){
            if(id && id != null){
        ...
        .then(function(result){
            ...
            session
                ...
                .then(function(result2){
                    ...
                    session
                        .run("OPTIONAL MATCH (a:Person)-[r:ACQUAINTANCES]-(b:Person) WHERE id(a)=toInt({idParam}) RETURN b", {idParam:id})
                        .then(function(result3){
                            var acquaintancesArr = []
                            
                            result3.records.forEach(function(record){
                                if(record._fields[0] != null){
                                    acquaintancesArr.push({
                                        id: record._fields[0].identity.low,
                                        name: record._fields[0].properties.name
                                    })
                                }
                            ...
                            res.render('person',{
                                ...
                                city:city,
                                state: state,
                                acquaintances:acquaintancesArr
                            })
